...
class DigitGUI:
    ...
    def setup_gui(self):
        ...
        self.create_live_preview_frame().grid(
            row=0, column=1, rowspan=2, padx=PADDING, pady=PADDING, sticky="nsew"
        )
        self.create_capture_controls_frame().grid(
            row=1, column=0, padx=PADDING, pady=PADDING, sticky="nsew"
        )
        self.create_save_dir_frame().grid(
            row=2, column=0, columnspan=2, padx=PADDING, pady=PADDING, sticky="ew"
        )
    ...
    def create_settings_frame(self):
        ...
        self.stream_combobox = ttk.Combobox(
            settings_frame, width=10, values=self.dc.get_stream_strings(), state="readonly"
        )
        ...
        # Create label
        num_frames_label = tk.Label(settings_frame, text="Number of Frames to Capture")
        ...
        # Create label
        interaction_num_label = tk.Label(settings_frame, text="Interaction Number")
        # Create a Spinbox to allow user to select interaction number
        interaction_num_validator = (
            self.root.register(self.validate_interaction_num),
            "%P",
        )
        ...
        # Create label
        countdown_secs_label = tk.Label(settings_frame, text="Countdown Seconds")
        # Create a Spinbox to allow user to select interaction number
        countdown_secs_validator = (self.root.register(self.validate_countdown_secs), "%P")
        self.countdown_secs_spinbox = tk.Spinbox(
            settings_frame,
            width=4,
            from_=1,
            to=MAX_COUNTDOWN_SECS,
            validate="key",
        ...
        # Create label
        force_level_label = tk.Label(settings_frame, text="Force Level")
        # Create a Spinbox to allow user to select interaction number
        force_level_validator = (self.root.register(self.validate_force_level), "%P")
        ...
        intensity_label.grid(
            row=0, column=0, sticky="ws", padx=PADDING, pady=PADDING
        )  # Align to bottom of slider
        self.intensity_slider.grid(
            row=0, column=1, sticky="w", padx=PADDING / 2, pady=PADDING
        )
        stream_label.grid(row=1, column=0, sticky="w", padx=PADDING, pady=PADDING)
        self.stream_combobox.grid(
            row=1, column=1, sticky="w", padx=PADDING / 2, pady=PADDING
        )
        num_frames_label.grid(row=2, column=0, sticky="w", padx=PADDING, pady=PADDING)
        self.num_frames_spinbox.grid(
            row=2, column=1, sticky="w", padx=PADDING / 2, pady=PADDING
        )
        interaction_num_label.grid(row=3, column=0, sticky="w", padx=PADDING, pady=PADDING)
        self.interaction_num_spinbox.grid(
            row=3, column=1, sticky="w", padx=PADDING / 2, pady=PADDING
        )
        countdown_secs_label.grid(row=4, column=0, sticky="w", padx=PADDING, pady=PADDING)
        self.countdown_secs_spinbox.grid(
            row=4, column=1, sticky="w", padx=PADDING / 2, pady=PADDING
        )
        force_level_label.grid(row=5, column=0, sticky="w", padx=PADDING, pady=PADDING)
        self.force_level_spinbox.grid(
            row=5, column=1, sticky="w", padx=PADDING / 2, pady=PADDING
        )
    ...
    def create_live_preview_frame(self):
        ...
        self.video_label.pack(padx=PADDING, pady=PADDING, anchor="center", expand=True)
    ...
    def create_capture_controls_frame(self):
        ...
        # Create a LabelFrame for whole section
        capture_controls_frame = tk.LabelFrame(
            self.root, text="Capture Controls", borderwidth=2, relief="groove"
        )
        ...
        self.save_button = tk.Button(
            capture_controls_frame, text="Capture", command=self.start_capture
        )
    ...
    def create_save_dir_frame(self):
        ...
        # Create a LabelFrame for whole section
        save_dir_frame = tk.LabelFrame(
            self.root, text="Save Directory", borderwidth=2, relief="groove"
        )
        ...
        self.save_dir_button = tk.Button(
            save_dir_frame, text="Select Directory", command=self.select_save_directory
        )

        # Pack the components into the save directory frame with padding
        # Ensure entry fills the width
        self.save_dir_entry.grid(row=0, column=0, sticky="ew", padx=PADDING, pady=PADDING)
        self.save_dir_button.grid(row=0, column=1, padx=PADDING, pady=PADDING)
    ...
    def try_connect_digit(self):
        ...
        if self.dc.digit is None:
            ...
            self.show_connection_failed_popup("DIGIT")
        else:
            # If connected, try and connect to serial
            self.try_connect_serial()

    def try_connect_serial(self):
        """Try to connect to the serial device. If it fails, show a popup."""

        # Create a SerialController instance to connect to the serial device
        self.sc = SerialController()
        # Check if the serial device is connected
        if self.sc.serial_connection is None:
            # If not connected, show a popup
            self.show_connection_failed_popup("Serial")
        else:
            # If connected, set up the GUI
            self.setup_gui()

    def show_connection_failed_popup(self, type):
        """
        Show a popup window if the connection fails to DIGIT or Serial with options to
        retry or exit.

        Args:
            type (str): The type of connection that failed ('DIGIT' or 'Serial').
        """
        ...
        failed_popup = DigitPopup(
            ...
            message=f"Failed to connect to a {type} device.",
            buttons=[
                ("Retry", lambda: self.retry_connection(failed_popup, type)),
                ("Exit", self.close_app),
            ],
        )
    ...
    def retry_connection(self, popup, type):
        """
        Retry the connection to DIGIT or Serial and close the popup.

        Args:
            popup (DigitPopup): The popup window to close.
            type (str): The type of connection to retry ('DIGIT' or 'Serial').
        """

        # Close the popup window
        popup.destroy()
        # Retry the relevant connection
        if type == "Serial":
            self.try_connect_serial()
        else:
            self.try_connect_digit()

    def show_lost_connection_popup(self, type):
        """
        Show a popup window when the connection to DIGIT or Serial is lost.

        Args:
            type (str): The type of connection that was lost ('DIGIT' or 'Serial').
        """
        ...
        lost_popup = DigitPopup(
            ...
            message=f"Lost connection to {type}.\nPlease reconnect and relaunch.",
    ...
    def update_video_frame(self):
        ...
        if self.view_running:
            try:
                ...
                self.disable_gui()
                self.show_lost_connection_popup("DIGIT")
    ...
    def select_save_directory(self):
        ...
        selected_dir = filedialog.askdirectory(
            initialdir=self.user_save_dir, title="Select Save Directory", mustexist=True
        )
    ...
    def start_countdown(self, seconds):
        ...
        self.capture_status_label.config(text=f"Capturing in {seconds} seconds...")
